...
export function PrismaAdapter(req: NextApiRequest | NextPageContext['req'], res: NextApiResponse | NextPageContext['res']): Adapter {
  return {
    async createUser(user: AdapterUser) {
      const { '@ysscall:userId': userIdOnCookies } = parseCookies({ req })
      ...
      if (!userExists) {
        throw new Error(`No user found with ID from cookie (${userIdOnCookies})`);
      }
      ...
      destroyCookie({ res }, '@ysscall:userId', {
        path: '/',
      })
